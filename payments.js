(()=>{
if(!document.querySelector('link[href*="payments.css"]')){const l=document.createElement('link');l.rel='stylesheet';l.href='payments.css?v=7';document.head.appendChild(l)}
if(!document.getElementById('openPayments')){const catCard=document.getElementById('cats')?.closest('.card');catCard?.insertAdjacentHTML('beforebegin','<div class="card"><div class="paymentQuickHead"><div><h2>Yaklaşan Ödemeler</h2><div class="sub">Son ödeme tarihlerini takip et</div></div><span id="paymentDueCount" class="dueCount"></span></div><div id="paymentQuickList" class="paymentQuickList"></div><div class="paymentToolbar"><button id="openPayments" class="soft" type="button">Ödeme Hatırlatmaları</button></div></div>')}
if(!document.getElementById('payments')){const listPage=document.getElementById('list');listPage?.insertAdjacentHTML('beforebegin',`<section id="payments" class="page"><div class="card"><div class="paymentPageHead"><div><h2>Ödeme Hatırlatmaları</h2><div id="paymentCountText" class="sub"></div></div><button id="backPayments" class="soft" type="button">Geri</button></div><div class="paymentInfo">iPhone'da güvenilir bildirim için ödeme kaydını oluşturduktan sonra <strong>Takvime Ekle</strong> düğmesine dokun. Oluşan takvim kaydının uyarısını iOS Takvim gönderir.</div><form id="paymentForm"><div class="paymentFormGrid"><div class="full"><label for="paymentName">Ödeme adı</label><input id="paymentName" maxlength="80" placeholder="Örn. Elektrik faturası" required></div><div><label for="paymentAmount">Tutar (₺)</label><input id="paymentAmount" inputmode="decimal" type="number" min="0.01" step="0.01" placeholder="0,00" required></div><div><label for="paymentDate">Son ödeme tarihi</label><input id="paymentDate" type="date" required></div><div><label for="paymentRepeat">Tekrar</label><select id="paymentRepeat"><option value="monthly">Her ay</option><option value="yearly">Her yıl</option><option value="once">Tek sefer</option></select></div><div><label for="paymentReminder">Hatırlatma</label><select id="paymentReminder"><option value="0">Aynı gün</option><option value="1" selected>1 gün önce</option><option value="3">3 gün önce</option><option value="7">7 gün önce</option></select></div><div><label for="paymentTime">Bildirim saati</label><input id="paymentTime" type="time" value="09:00"></div><div class="full"><label for="paymentNote">Açıklama</label><input id="paymentNote" maxlength="120" placeholder="Örn. Otomatik ödeme yok"></div></div><div class="formActions"><button class="primary" type="submit">Ödemeyi Kaydet</button></div></form></div><div class="card"><div class="sectionHead"><div><h2>Planlanan Ödemeler</h2><div class="sub">Yaklaşan ve geciken ödemeler</div></div></div><div id="paymentItems" class="paymentList"></div><div class="paymentToolbar"><button id="clearPayments" class="danger" type="button">Tüm Ödeme Planını Sil</button></div></div></section>`)}
